...
class TD3(object):
    def __init__(self,
        ...
        self.actor_target = build_td3_actor_model(actor_input_dim, action_dim, max_action, init_rng)
        actor_optimizer = optim.Adam(learning_rate=3e-4).create(actor)
        ...
        critic_optimizer = optim.Adam(learning_rate=3e-4).create(critic)
    ...
    def sample_action(self, state):
        return self.select_action(state) + \
            self.max_action * self.expl_noise * random.normal(next(self.rng), self.action_dim)
    ...
    def train(self, replay_buffer, batch_size=100):
        ...
        target_Q = jax.lax.stop_gradient(no_grad(next(self.rng),
                                         *buffer_out, *self.target_params))

        state, action, _, _, _ = buffer_out

        self.critic_optimizer = critic_step(self.critic_optimizer, state, action, target_Q)

        if self.total_it % self.policy_freq == 0:

            self.actor_optimizer = actor_step(self.actor_optimizer, self.critic_optimizer, state)

            self.critic_target = copy_params(self.critic_optimizer.target, self.critic_target, self.tau)
            self.actor_target = copy_params(self.actor_optimizer.target, self.actor_target, self.tau)
